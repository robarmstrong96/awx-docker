...
def test_public_readiness_forbidden_pattern_fails(tmp_path: Path) -> None:
    ...
    report = scan_public_readiness(tmp_path, policy, tmp_path / "evidence")

    assert report.status == "fail"
    assert report.findings[0].name == "local-home-path"


def test_public_readiness_allowed_path_exception(tmp_path: Path) -> None:
    (tmp_path / "README.md").write_text("ok\n")
...
def test_public_readiness_missing_required_file_fails(tmp_path: Path) -> None:
    policy = tmp_path / "policy.yml"
    write_policy(policy)
